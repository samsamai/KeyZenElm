...
Elm.KeyZenElmModel = Elm.KeyZenElmModel || {};
Elm.KeyZenElmModel.make = function (_elm) {
   "use strict";
   _elm.KeyZenElmModel = _elm.KeyZenElmModel || {};
   if (_elm.KeyZenElmModel.values) return _elm.KeyZenElmModel.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var wordArray = $Array.fromList(_U.list(["rake db:migrate","rake db:reset","rails s","rails c","binding.pry","{h:\'a\', i:\'b\'}"]));
   var nextWord = function (current_index) {    var index = current_index;return A2($Maybe.withDefault,"",A2($Array.get,current_index,wordArray));};
   var seed = $Random.initialSeed(4321);
   var x = function (model) {    return A2($Maybe.withDefault,_U.chr(" "),A2($Array.get,model.char_index,model.training_chars));};
   var randomInt = F2(function (model,old_model) {
      var _p0 = A2($Random.generate,A2($Random.$int,0,$Array.length(old_model.training_chars)),old_model.next_seed);
      var i = _p0._0;
      var s = _p0._1;
      return _U.update(model,{next_seed: s,char_index: i});
   });
   var intList = A2($Random.list,5,A2($Random.$int,0,100));
   var RandState = F2(function (a,b) {    return {seed: a,$char: b};});
   var WordState = F7(function (a,b,c,d,e,f,g) {    return {sample: a,training_chars: b,typed: c,current_char: d,current_word: e,char_index: f,next_seed: g};});
   var seed0 = $Random.initialSeed(42);
   var randState0 = {seed: seed0,$char: _U.chr("_")};
   var gen = A2($Random.$int,0,100);
   var wordLength = 7;
   var chars = $Array.fromList($String.toList(" jfkdlsahgyturieowpqbnvmcxz6758493021`-=[]\\;\',./ABCDEFGHIJKLMNOPQRSTUVWXYZ~!@#$%^&*()_+{}|:\"<>?"));
   var model0 = {sample: $Array.fromList(_U.list([]))
                ...
                ,next_seed: seed0};
   var nextRandomWord = function (model) {    return A2($Array.map,x,$Array.fromList(A3($List.scanl,randomInt,model0,A2($List.repeat,10,model0))));};
   return _elm.KeyZenElmModel.values = {_op: _op
                                       ,chars: chars
                                       ...
                                       ,WordState: WordState
                                       ,RandState: RandState
                                       ,randState0: randState0
                                       ,intList: intList
                                       ,randomInt: randomInt
                                       ,nextRandomWord: nextRandomWord
                                       ,x: x
                                       ,seed: seed
                                       ,wordArray: wordArray
                                       ,model0: model0
...
Elm.KeyZenElmView = Elm.KeyZenElmView || {};
Elm.KeyZenElmView.make = function (_elm) {
   "use strict";
   _elm.KeyZenElmView = _elm.KeyZenElmView || {};
   if (_elm.KeyZenElmView.values) return _elm.KeyZenElmView.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $KeyZenElmModel = Elm.KeyZenElmModel.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var untypedChar = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "#EEEEEE"}]));
   var goodChar = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "#AAAAAA"}
                                                 ,{ctor: "_Tuple2",_0: "text-shadow",_1: "0px 1px 1px #FFF, 0px 2px 2px #FFF"}]));
   var errorChar = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "#FF0000"}]));
   var currentChar = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "border-bottom",_1: "4px solid #f78d1d"}]));
   var normalChar = $Html$Attributes.style(_U.list([]));
   var word = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: "#AAA"}
                                             ,{ctor: "_Tuple2",_0: "font-size",_1: "28px"}
                                             ...
                                             ,{ctor: "_Tuple2",_0: "text-shadow",_1: "0px 2px 3px #000"}]));
   var input_style = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"}
                                                    ,{ctor: "_Tuple2",_0: "height",_1: "68px"}
   ...
   var myStyle = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "100%"}
                                                ...
                                                ,{ctor: "_Tuple2",_0: "font-size",_1: "2em"}
                                                ,{ctor: "_Tuple2",_0: "text-align",_1: "center"}]));
   var body = $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background-image",_1: "url(\'brushed_alu.png\')"}
                                             ,{ctor: "_Tuple2",_0: "font-family",_1: "\'Ubuntu Monokz\', sans-serif"}
                                             ,{ctor: "_Tuple2",_0: "padding",_1: "0"}
                                             ,{ctor: "_Tuple2",_0: "margin",_1: "0"}
                                             ,{ctor: "_Tuple2",_0: "min-height",_1: "100%"}
                                             ,{ctor: "_Tuple2",_0: "outline",_1: "none"}]));
   var sample_char = F2(function (index,model) {
      var char_class = _U.cmp(index,model.current_char) > 0 ? normalChar : _U.eq(index,model.current_char) ? currentChar : _U.eq(A2($Array.get,
      index,
      model.sample),
      A2($Array.get,index,model.typed)) ? goodChar : !_U.eq(A2($Array.get,index,model.sample),A2($Array.get,index,model.typed)) ? errorChar : untypedChar;
      return A2($Html.span,_U.list([char_class]),_U.list([$Html.text($String.fromChar(A2($Maybe.withDefault,_U.chr(" "),A2($Array.get,index,model.sample))))]));
   });
   var sample_word = function (m) {
      return A2($Html.div,
      _U.list([myStyle]),
      A3($Array.foldr,
      F2(function (x,acc) {    return A2($List._op["::"],A2(sample_char,x,m),acc);}),
      _U.list([]),
      $Array.fromList(_U.range(0,$Array.length(m.sample) - 1))));
   };
   var createNextModel = function (old_model) {
      var _p0 = A2($Random.generate,A2($Random.$int,0,500),old_model.next_seed);
      var seed$ = _p0._1;
      var new_model = _U.update($KeyZenElmModel.model0,{next_seed: seed$});
      return _U.update(new_model,{sample: $KeyZenElmModel.nextRandomWord(new_model),next_seed: seed$});
   };
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.div,
      _U.list([word]),
      _U.list([sample_word(model)
              ,A2($Html.input,
              _U.list([$Html$Attributes.placeholder("")
                      ,$Html$Attributes.value($String.fromList($Array.toList(model.typed)))
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,A2(makeMessage,address,model))
                      ,$Html$Attributes.autofocus(true)
                      ,input_style]),
              _U.list([]))
              ,A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model.current_char))]))]))]));
   });
   var update = F2(function (new_model,old_model) {    return new_model;});
   return _elm.KeyZenElmView.values = {_op: _op
                                      ,update: update
                                      ,view: view
...
Elm.KeyZenElm = Elm.KeyZenElm || {};
Elm.KeyZenElm.make = function (_elm) {
   "use strict";
   _elm.KeyZenElm = _elm.KeyZenElm || {};
   if (_elm.KeyZenElm.values) return _elm.KeyZenElm.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $KeyZenElmModel = Elm.KeyZenElmModel.make(_elm),
   $KeyZenElmView = Elm.KeyZenElmView.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Random = Elm.Random.make(_elm),
   $Result = Elm.Result.make(_elm),
   ...
   var firstSeed = $Random.initialSeed($Basics.round(startTime));
   var main = $StartApp$Simple.start({model: _U.update($KeyZenElmModel.model0,{next_seed: firstSeed}),view: $KeyZenElmView.view,update: $KeyZenElmView.update});
   return _elm.KeyZenElm.values = {_op: _op,firstSeed: firstSeed,main: main};
};
